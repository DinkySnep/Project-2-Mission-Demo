...
public class FollowCam : MonoBehaviour
{
    ...
    public Vector2 minXY = Vector2.zero;    // this is [0,0]
    ...
    void FixedUpdate()
    {
        ...
        if (POI != null)
        {
            // If the POI has a rigidboddy check to see if it is sleeping
        ...
        // limit the minn values of the destinaiton x and y cords
        destination.x = Mathf.Max(minXY.x, destination.x);
        destination.y = Mathf.Max(minXY.y, destination.y);
        // interpolate from the current camera position twoard destiniation
